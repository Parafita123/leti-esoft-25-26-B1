...
- No dependencies 
- However, employee must be previously created with assigned role ([US14](../US14/US14.md)).
...
|:----:|-------------------------------------------------------------------------------|----------------------------------|--------------|
| 1    | interacting with the actor?                                                   | ListEmployeesByRoleView          | **Pure Fabrication / IE** – UI class handles all user interaction |
| 2    | presenting the available roles and requesting the Administrator’s selection?  | ListEmployeesByRoleView          | UI responsibility for data input/output |
| 3    | requesting the list of employees filtered by the selected role?               | ListEmployeesByRoleController    | **Controller pattern** – mediates between UI and domain logic |
| 4    | searching employees with the specified role and returning the resulting list? | EmployeeContainer                | **Information Expert** – owns and manages the employee collection |
