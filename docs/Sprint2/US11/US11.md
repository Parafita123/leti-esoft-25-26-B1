...
> **Question:** Are there any other mandatory attributes when registering a vaccine?  
> **Answer:** The vaccine must be described by its commercial name and brand.  The target disease may be inferred from the selected vaccine type.
...
Three relevant test scenarios are highlighted below (additional tests should be defined in the implementation).

1. **Test 1:** Check that creating a Vaccine instance with an invalid technology (not part of the predefined list) throws an exception.

    ```C++
    TEST_F(VaccineFixture, CreateWithInvalidTechnology){
        EXPECT_THROW(new Vaccine(L"COVIDShield", L"BioPharma", L"unknown"), std::invalid_argument);
    }
    ```

2. **Test 2:** Check that creating a Vaccine instance with valid data (commercial name, brand and a valid technology) does not throw.

    ```C++
    TEST_F(VaccineFixture, CreateWithValidData){
        EXPECT_NO_THROW(new Vaccine(L"COVIDShield", L"BioPharma", L"mRNA"));
    }
    ```

3. **Test 3:** Check that it is possible to create and save a vaccine via the VaccineContainer, and that the collection is updated.

    ```C++
    TEST_F(VaccineContainerFixture, AddingOneVaccine){
        EXPECT_TRUE(this->container->isEmpty());
        std::shared_ptr<Vaccine> vac = this->container->create(L"COVIDShield", L"BioPharma", L"mRNA");
        this->container->save(vac);
...
- **Repository/Service:** Implement `VaccineContainer` with in‑memory storage and validation of technology.  Inject it into the controller.
- **View and Controller:** Define `RegisterVaccineView` and `RegisterVaccineController` following the patterns used in previous user stories.
- **Domain:** Implement the `Vaccine` class with a constructor that validates its fields; implement `VaccineType` separately.

## 6. Integration and Demo

Add a new menu option in the console application accessible only to administrators.  When selected, it should instantiate `RegisterVaccineView` and start the process of registering a vaccine.

## 7. Observations

- Additional acceptance criteria may be identified during implementation (e.g. uniqueness across different lots, expiration dates).
